...
#' @title Perform Currency Check for Data Frame Columns
...
#' it Checks currency rules (Temporal conditions) on columns of a data frame, based on metadata specifications.
...
#' @param M_data data.frame. Metadata describing variables and their currency rules. Must include columns \code{VARIABLE}, \code{Currency_Rule} and \code{TYPE}. Optionally includes \code{Currency_Error_Type}.
...
#'   \item \strong{Currency_Rule}: A logical rule provided as a string that defines temporal (date/time) conditions to be evaluated.
#'   \item \strong{TYPE}: Specifies the type of the variable (e.g., "numeric", "date", "character").
#'   \item \strong{Currency_Error_Type}: The error type for each rule will be reported in the summary output.Based on the importance and severity of the rule, it can include two options: "Warning" or "Error".
#' }
#'
#'
...
#'   \item Currency_Error_Type: Error type from metadata (if available).
#' }
...
currency_check <- function(
  ...
  required_meta_cols <- c("VARIABLE", "Currency_Rule", "TYPE")
  if (!all(required_meta_cols %in% names(M_data))) stop("M_data must contain columns: VARIABLE, Currency_Rule, TYPE.")
  ...
  for (i in seq_len(nrow(M_sub))) {
    ...
    rule_raw <- as.character(M_sub$Currency_Rule[i])
    ...
    rule_list[[length(rule_list) + 1]] <- list(
      ...
      error_type = if ("Currency_Error_Type" %in% names(M_sub)) M_sub$Currency_Error_Type[i] else NA
    )
  }
  ...
  if (isTRUE(Result)) {
    ...
    # When Result = FALSE: summary table, include Currency_Error_Type if present
    summary_list <- lapply(names(res_list), function(vn) {
      ...
      error_type <- NA
      if ("Currency_Error_Type" %in% names(M_sub)) {
        row_match <- which(M_sub$VARIABLE == vn)
        if (length(row_match) > 0) error_type <- M_sub$Currency_Error_Type[row_match[1]]
      }
      data.frame(
        VARIABLE = vn,
        Condition_Met = met,
        ...
        Currency_Error_Type = error_type,
        stringsAsFactors = FALSE
      )
    })
